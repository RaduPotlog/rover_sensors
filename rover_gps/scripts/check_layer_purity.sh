...
#
# Fails if a file in the given layer includes a ROS or socket header, or an in-project header
...
# socket); this makes the rule explicit and catches header-only leaks the build would not.
...
FORBIDDEN_KEYWORDS=(
    ...
    geometry_msgs
    std_msgs
    # The UDP receiver is infrastructure; domain parses bytes it is handed.
    sys/socket
    netinet/
    arpa/inet
)
